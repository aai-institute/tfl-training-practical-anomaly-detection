...
class Density_model:
    ...
    def fit(self, X, X_plot, grid, k="auto", norm_mom=True, hstd_mom=False):
        ...
        """
        if self.algo == "kde":
            self.density, self.model = kde_lib.kde(
                X, X_plot, self.bandwidth, self.kernel, return_model=True
            )
        elif self.algo == "mom-kde":
            self.n_block = k
            self.density, self.model = kde_lib.mom_kde(
                X,
                X_plot,
                self.bandwidth,
            ...
            self.X_data = X
            self.density, self.model = kde_lib.rkde(
                X, X_plot, self.bandwidth, type_rho="hampel", return_model=True
            )
        elif self.algo == "rkde-huber":
            self.X_data = X
            self.density, self.model = kde_lib.rkde(
                X, X_plot, self.bandwidth, type_rho="huber", return_model=True
            )
        elif self.algo == "spkde":
            self.X_data = X
            self.density, self.model = kde_lib.spkde(
                X, X_plot, self.bandwidth, self.outliers_fraction, return_model=True
            )
        else:
    ...
    def compute_score(self, true_dens):
        ...
        self.kullback_f0_f = metrics.kl(
            true_dens.reshape((-1, 1)), self.density.reshape((-1, 1))
        )
        self.kullback_f_f0 = metrics.kl(
            self.density.reshape((-1, 1)), true_dens.reshape((-1, 1))
        )
        self.jensen = metrics.js(
            self.density.reshape((-1, 1)), true_dens.reshape((-1, 1))
        )
        ...
        if self.algo == "kde":
            ...
            for i_d in range(d):
                ...
                K_plot = (
                    K_plot
                    + (
                        np.dot(np.ones((m, 1)), temp_x.T)
                        - np.dot(temp_xpos, np.ones((1, n_samples)))
                    )
                    ** 2
                )
