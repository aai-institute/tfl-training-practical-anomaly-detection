...
def load_data(dataset):
    ...
    print(
        "Loaded {} data: {} samples, {} dimensions, {} labels".format(
...
def balance_outlier(X, y, e=0.1):
    ...
    e :  epsilon to use for outlier sampling. The number of outliers will be equal to e * n_inlier.
    """
    if e == 0:
        X = X.copy()
        y = y.copy()
...
def generate(
    ...
    if outlier_type == "gaussian":
        X = np.r_[
            X,
            np.random.normal(
                loc=outlier_param[0], scale=outlier_param[1], size=(n_outliers, dim)
            ),
        ]
    # Dirac
    elif outlier_type == "dirac":
        X = np.r_[
            X, np.random.normal(loc=outlier_param[0], scale=0, size=(n_outliers, dim))
        ]
    # Uniform outliers
        ...
        X = np.r_[
            X,
            np.random.uniform(
                low=outlier_param[0], high=outlier_param[1], size=(n_outliers, dim)
            ),
        ]
        ...
        X = np.r_[
            X,
            np.random.uniform(
                low=outlier_param[0], high=outlier_param[1], size=(n_outliers // 2, dim)
            ),
        ]
        X = np.r_[
            X,
...
def generate_situations(n_samples, outlier_prop, outlier_scheme="uniform", dim=1):
    """
...
def true_density(gauss_param, X_plot):
    ...
    if d > 1:
        true_dens = coeff * sci_multinorm(
            (gauss_param[0], gauss_param[0]), gauss_param[2]
        ).pdf(X_plot) + coeff * sci_multinorm(
            (gauss_param[1], gauss_param[1]), gauss_param[3]
        ).pdf(
            X_plot
        )
    return true_dens
